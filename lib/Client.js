...
const {defaultSockPath,EVENT_NEW_MESSAGE} = require('./common_util');
/**
 * 0字节 option
 * 1-4 字节 msg_id
 * 5-8字节 消息长度
 ...
 * @param {Number} [responseTimeout] - 请求超时
   ...
   */
    constructor(options) {
      ...
      options = Object.assign({}, defaultOptions, options);
        ...
        const _this = this;
        /**
         * @param {NewMessageEventCallback}
         */
        this.on(EVENT_NEW_MESSAGE, function newMessageCallback(message, reply, client) {
          if (message && typeof message.action === 'string') {
            _this.emit(message.action, message.data, reply, client);
          }
        });
    }
    ready(callback) {
