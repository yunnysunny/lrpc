...
describe('test/client_test.js', () => {
    ...
    it('should emit close in the same tick', function(done) {//TODO 
        let client = new Client({
            name,
        });
    ...
    it.skip('should reconnect after packet parsed error and invoke ok', done => {
  ...
  it('should reconnect and emit error if still can\'t connect / 2', done => {
    ...
    it.skip('should emit error if parse header error', done => {
        let client = new Client({
            name,
        });
    ...
    it.skip('should emit error if parse body error', done => {
      class ErrorBodyLenClient extends Client {
        getBodyLength() {
          throw new Error('body');
        }
      }
        let client = new ErrorBodyLenClient({
            name,
        });


        uncaughtException(err => {
            assert(err.message === 'PacketParsedError');
            done();
        });
